...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/30 13:28:29 by tmalheir          #+#    #+#             */
/*   Updated: 2023/12/01 11:09:23 by tmalheir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (NULL);
	if (!st_buffer[fd])
		st_buffer[fd] = ft_strdup("");
	st_buffer[fd] = read_line(fd, buffer, st_buffer[fd]);
	line = NULL;
	line = get_line(st_buffer[fd], line);
	st_buffer[fd] = get_remain(st_buffer[fd]);
	if (!st_buffer[fd] && ft_strlen(line) == 0)
	{
		free (line);
		return (NULL);
	}
